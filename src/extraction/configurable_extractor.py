...
class ConfigurableExtractor:
    ...
    def __init__(self, config: Optional[ExtractionConfig] = None):
        ...
        # Initialize Docling extractor (open-source)
        self.docling_extractor = None
        if self.config.enable_docling:
            try:
                from src.extraction.docling_extractor import DoclingExcelExtractor
                self.docling_extractor = DoclingExcelExtractor(self.config)
                self.logger.info("Docling extractor initialized")
            except ImportError:
                self.logger.warning("Docling extractor not available (pip install docling)")
        
        # Initialize Unstructured.io extractor
        self.unstructured_extractor = None
        if self.config.enable_unstructured:
            try:
                from src.extraction.unstructured_extractor import UnstructuredExcelExtractor
                self.unstructured_extractor = UnstructuredExcelExtractor(self.config)
                self.logger.info("Unstructured.io extractor initialized")
            except ImportError:
                self.logger.warning("Unstructured extractor not available (pip install 'unstructured[xlsx]')")
    ...
    async def _extract_with_strategy(
        ...
        if strategy == ExtractionStrategy.OPENPYXL:
            ...
            if not self.docling_extractor:
                raise ValueError("Docling extractor not enabled or available")
            return await self.docling_extractor.extract_workbook(
                file_content, file_id, file_name, file_path, modified_time
            )
        
        elif strategy == ExtractionStrategy.UNSTRUCTURED:
            if not self.unstructured_extractor:
                raise ValueError("Unstructured extractor not enabled or available")
